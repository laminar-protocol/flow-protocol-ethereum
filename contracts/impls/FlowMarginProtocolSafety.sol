...
contract FlowMarginProtocolSafety is Initializable, UpgradeOwnable, UpgradeReentrancyGuard {
    ...
    FlowMarginProtocol private marginProtocol;
    ...
    modifier poolIsVerified(LiquidityPoolInterface _pool) {
        require(marginProtocol.liquidityPoolRegistry().isVerifiedPool(_pool), "LR1");

        _;
    }

    /**
     * @dev Initialize the FlowMarginProtocolSafety.
     ...
     */
    function initialize(
        FlowMarginProtocol _marginProtocol,
    ...
    */
    ...
    function marginCallTrader(LiquidityPoolInterface _pool, address _trader) external nonReentrant poolIsVerified(_pool) {
        ...
        marginProtocol.moneyMarket().baseToken().safeTransfer(msg.sender, marginProtocol.TRADER_MARGIN_CALL_FEE());
    ...
    function makeTraderSafe(LiquidityPoolInterface _pool, address _trader) external nonReentrant poolIsVerified(_pool) {
        ...
        marginProtocol.moneyMarket().baseToken().safeTransferFrom(msg.sender, address(this), marginProtocol.TRADER_MARGIN_CALL_FEE());
    ...
    function marginCallLiquidityPool(LiquidityPoolInterface _pool) external nonReentrant poolIsVerified(_pool) {
        ...
        marginProtocol.moneyMarket().baseToken().safeTransfer(msg.sender, marginProtocol.liquidityPoolRegistry().LIQUIDITY_POOL_MARGIN_CALL_FEE());
    ...
    function makeLiquidityPoolSafe(LiquidityPoolInterface _pool) external nonReentrant poolIsVerified(_pool) {
        ...
        marginProtocol.moneyMarket().baseToken().safeTransferFrom(
            msg.sender,
            address(this),
            marginProtocol.liquidityPoolRegistry().LIQUIDITY_POOL_MARGIN_CALL_FEE()
        );
    ...
    function liquidateTrader(LiquidityPoolInterface _pool, address _trader) external nonReentrant poolIsVerified(_pool) {
        ...
        uint256 positionsLength = marginProtocol.getPositionsByPoolAndTraderLength(_pool, _trader);
        ...
        marginProtocol.moneyMarket().baseToken().safeTransferFrom(address(this), msg.sender, marginProtocol.TRADER_LIQUIDATION_FEE());
    ...
    */
    function liquidateLiquidityPool(LiquidityPoolInterface _pool) external nonReentrant poolIsVerified(_pool) {
        ...
        uint256 positionsLength = marginProtocol.getPositionsByPoolLength(_pool);

        for (uint256 i = 0; i < positionsLength; i++) {
            (uint id,,,FlowMarginProtocol.TradingPair memory pair,int leverage,int leveragedHeld,,,,,) = marginProtocol.positionsByPool(_pool, i);
            _liquidityPoolClosePosition(_pool, id, pair, leverage, leveragedHeld);
        }
        ...
        marginProtocol.moneyMarket().baseToken().safeTransferFrom(
            address(this),
            msg.sender,
            marginProtocol.liquidityPoolRegistry().LIQUIDITY_POOL_LIQUIDATION_FEE()
        );
    ...
    function _getEnpAndEll(LiquidityPoolInterface _pool) internal returns (Percentage.Percent memory, Percentage.Percent memory) {
        ...
        for (uint256 i = 0; i < positionsLength; i++) {
            int256 leveragedDebitsInUsd = marginProtocol.getLeveragedDebitsByPoolAndIndex(_pool, i);
    ...
    function _liquidityPoolClosePosition(
        ...
        uint256 _id,
        FlowMarginProtocol.TradingPair memory _pair,
        int256 _leverage,
        int256 _leveragedHeld
    ) internal returns (uint256) {
        Percentage.Percent memory price = marginProtocol.getPrice(_pair.base, _pair.quote);
        uint256 spread = _leverage > 0
            ? marginProtocol.getBidSpread(_pool, address(_pair.base))
            : marginProtocol.getAskSpread(_pool, address(_pair.quote));

        uint256 leveragedHeldAbs = _leveragedHeld >= 0 ? uint256(_leveragedHeld) : uint256(-_leveragedHeld);
        uint256 spreadProfit = leveragedHeldAbs.mul(spread.mulPercent(price));
        uint256 spreadProfitInUsd = uint256(marginProtocol.getUsdValue(_pair.base, int256(spreadProfit)));
    ...
    function _getLeveragedDebitsOfTrader(LiquidityPoolInterface _pool, address _trader) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < positionsLength; i++) {
            int256 leveragedDebitsInUsd = marginProtocol.getLeveragedDebitsByPoolAndTraderAndIndex(_pool, _trader, i);
    ...
    function _getEquityOfPool(LiquidityPoolInterface _pool) internal returns (int256) {
        uint256 liquidity = marginProtocol.moneyMarket().convertAmountToBase(marginProtocol.moneyMarket().iToken().balanceOf(address(_pool)));
        ...
        for (uint256 i = 0; i < positionsLength; i++) {
            uint256 positionId = marginProtocol.getPositionIdByPoolAndIndex(_pool, i);
            int256 unrealized = marginProtocol.getUnrealizedPlOfPosition(positionId);
            uint256 accSwapRate = marginProtocol.getAccumulatedSwapRateOfPosition(positionId);

            unrealizedPlAndRate = unrealizedPlAndRate.add(unrealized).add(int256(accSwapRate));
        }
